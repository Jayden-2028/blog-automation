...
마지막 세션: 2026-09-18 — 웹 검색 이미지 수집을 파이프라인에 연결(Codex→Claude WebSearch),
이미지 실사 기본 전환, 텔레그램 더블탭 물리 차단
...
- [x] 웹 검색 이미지 수집을 파이프라인에 연결 — Claude WebSearch + Storage 업로드, 승인 직후 자동 (2026-09-18)
- [x] AI 이미지 기본을 실사로 — 일러스트는 도식 예외만, 검수로 탐지 (2026-09-18)
- [x] 텔레그램 더블탭 물리 차단 — Worker가 누른 즉시 버튼을 '처리 중'으로 교체 (2026-09-18)
- [x] 표·인포그래픽 자리를 직접 렌더 — `— 표 생성`, 본문 표·목록 → Chromium → PNG (2026-09-18)
- [x] 네이버용 배리에이션 원고 — 이미지 공유(마커 불변 검증), 뷰어에서만 생성·복사 (2026-09-18)
