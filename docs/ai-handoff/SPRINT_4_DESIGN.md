...
작성일: 2026-08-28 · 상태: **승인 대기** (§9에 결정 항목 6건)
...
- **발행량 상한**: 로드맵 §6-3(저품질 판정 위험)에 따라 하루 임시저장 건수를 코드로 제한한다.
  `publications`에서 오늘자 row 수를 세고, 상한(기본 2건, 환경변수로 조정) 초과 시 거부하고
  Telegram으로 안내한다.
...
1. **트리거 방식**(§7): CLI만 / CLI + Telegram 버튼 동시 — 권고: CLI 먼저, 안정화 후 버튼.
2. **발행 대상 블로그**: `CREATOR_ADVISOR_BLOG_ID`와 같은 블로그인가? (Creator Advisor가
   통계를 보는 블로그에 실제로 글을 쓰는 것이 맞는지 확인 필요)
3. **일일 발행 상한 값**: 기본 2건 제안(로드맵 §6-3 그대로) — 조정 여부.
4. **쓰기 세션 최초 로그인**: 사용자가 직접 새 프로필로 1회 수동 로그인해야 한다(Creator
   Advisor 때와 동일한 방식) — 진행 시점 확인 필요.
5. **이미지 위치 타협안**(§6-4): 정확한 섹션별 위치 재현이 실측에서 안 되면 "본문 끝에 몰아
   넣기"로 타협해도 되는지, 아니면 위치 재현이 될 때까지 이 스프린트를 안 끝낸 것으로 볼지.
6. **실측 단계를 별도로 승인받을지**: §10의 1번(Playwright로 실제 로그인된 브라우저를 열어
   SmartEditor DOM을 살펴보는 것)부터가 사용자 네이버 계정의 실제 세션을 여는 작업이다.
   Creator Advisor 때처럼 사람이 지켜보는 가운데(`headless: false`) 진행할지 확인 필요.

## 10. 작업 순서

| # | 작업 | 담당 | 선행 |
|---|---|---|---|
| 1 | 실측: SmartEditor 글쓰기 URL·셀렉터·이미지 업로드 방식·저장/발행 버튼 구분 확인(`debug:naver-publish` 스크립트, `--inspect` 모드) | Claude + 사용자 | §9 결정 |
