...
| # | 작업 | 담당 | 선행 |
|---|---|---|---|
| 1 | 실측(진행 중, 2026-08-28 맥 사용 어려움으로 중단 - 재개 대기): 최초 로그인 + 데스크톱 SmartEditor·모바일 웹 에디터(§6-1) 둘 다 DOM 확인해 자동화 대상 확정 + 셀렉터·이미지 업로드 방식·저장/발행 버튼 구분(`npm run setup:naver-publish`) | Claude + 사용자 | 사용자가 맥 앞에 있을 때 |
