...
- **남은 구조적 한계**: Codex는 URL을 고를 뿐 **그 이미지를 실제로 보지 않는다.** 대만 OTT에서
  가져온 나비 이미지는 `alt`에 "공식 포스터"라고 적혀 있지만 실제 파일은 스틸컷이다. 설명과 실제
  내용이 어긋나는 이 유형은 다운로드 후 비전 검증 단계를 붙여야 잡힌다(미구현).
