...
**Sprint 0·1 완주, Sprint 2 완료, Sprint 3(검수 게이트) 1~4단계 완료.** 매일 09:00 키워드
TOP 10이 Telegram으로 오고, Go/Pass 버튼으로 선택하면 `article_jobs`가 생긴다. 거기서부터
조사 완료 알림의 `[✍️ 원고 작성][🗑 중단]` 버튼으로 원고 생성(Telegraph 발행, 해시태그 15개,
저신뢰 출처 고지)까지 이어지고, 이제 원고가 나오면 **검수 규칙 4종이 자동으로 돌아 결과가
알림에 표시**되며(차단은 아님), `[✅ 승인][✏️ 수정 필요][🗑 반려]` 버튼(모든 원고 공통)으로
`job`/`article` status가 `approved`로 전이된다. 경복궁·아기 셔더링어택·재혼 황후·보조금24
4건으로 전 구간 실측 검증 완료. 상세는 `docs/ai-handoff/SPRINT_2_DESIGN.md` 14절과
`docs/ai-handoff/SPRINT_3_DESIGN.md` 14절.

**다음 작업: Sprint 3 이미지(5~6단계).** `docs/ai-handoff/SPRINT_3_DESIGN.md` §9~13 —
Claude가 이미지 브리프(장면+생성 프롬프트+alt_text+금지 항목)를 만들어 Telegram으로 보내면
사용자가 ChatGPT로 확보하고, 결과를 `images` 테이블에 기록한다(이때 `images.id`가
identity인지 첫 insert로 검증). 서치는 공식 보도자료·라이선스 스톡으로만 한정, 생성이 기본.
...
## Sprint 3: 검수 게이트 + 이미지 (1~4단계 완료, 이미지 대기)

설계: `docs/ai-handoff/SPRINT_3_DESIGN.md`. 승인된 결정 5건은 §15에, 작업 순서/상태는 §14에 있다.

**구현 완료(커밋 `83542f6`, `f249b66`) + 실측 검증(2026-08-28, "보조금24" job)**:
...
**남은 것(5~6단계)**: 이미지는 **ChatGPT 수동 핸드오프**다 - Codex CLI는 코딩 에이전트라 이미지
생성이 불가하다. Claude가 브리프(장면 + 생성 프롬프트 + alt_text + 금지 항목)를 만들어 Telegram
으로 보내고, 사용자가 ChatGPT에서 확보한 뒤 업로드 직전 직접 검수한다. 서치는 공식 보도자료·
라이선스 명시 스톡으로만 한정, 생성이 기본(결정 5번 - 웹 검색으로 찾은 이미지는 로드맵 §6-1이
"가장 큰 실질 위험"으로 지목한 타인 저작물이고, 사람이 봐도 침해 여부가 판별되지 않는다). 저장은
기존 `images` 테이블을 쓴다 - 첫 insert 시 `images.id`가 identity인지 검증해야 한다.
