...
**한 줄**: 규칙은 맞았는데 **읽는 사람이 없었다.** 카테고리별 이미지 규칙(§8-7)을
`output-format.md`에 적어 뒀지만 그 문서를 읽는 건 집필자뿐이고, 실제로 검색하는
`collectWebImages` 프롬프트에는 카테고리도 사이트 우선순위도 없었다.

### 새로 생긴 것

| 파일 | 역할 |
|---|---|
| `images/imageSearchPools.ts` | 카테고리별 서치풀(작품: 공식 채널 → 키노라이츠 → 출연자 SNS) |
| `images/searchKinolightsStills.ts` | 키노라이츠 공식 스틸 직접 수집(제목 → 작품 페이지 → 미디어 섹션) |
| `images/tableVisuals.ts` | 인포그래픽 픽토그램·막대그래프 |
| `images/imageFingerprint.ts` | 같은 컷 차단(바이트 해시 + dHash + 비전) |
| `config/naverContentUrl.ts` | 네이버 블로그 본문을 모바일 주소로 받는다 |
| `config/writingMode.ts` | spec/auto 규격 모드 스위치 |
| `rules/article-structure.md` | 카테고리별 문단 흐름 8개 |
| `rules/topic-allocation.md` | 내적 60% / 외적 40% / 외적의 외적 0% |

### 되풀이한 실수 — 다음 세션이 꼭 볼 것

**만들어 놓고 읽는 쪽을 배선하지 않는 결함을 하루에 네 번 반복했다.**

```
인스타 sourceContext   auto 분기에 없었다
이미지 지문            page.evaluate 인자 미전달로 항상 null (fail-open이라 조용히 통과)
캡션                   record.caption을 아무도 안 읽었다
중복 차단              이미 채워진 자리를 등록하지 않아 4번 연속 안 돌았다
```

전부 **단위 테스트는 통과했는데 운영 조건에서 안 돌았다.** 그래서 이제:

- 값을 만들면 **끝까지 도달하는지** 테스트한다(`test:caption-wiring`이 그 예 -
  고치기 전 코드로 되돌려 테스트가 실패하는 것까지 확인했다).
- **fail-open 경로에는 반드시 로그를 남긴다.** 지문 실패는 흔적이 없어 원인을
  특정하는 데 오래 걸렸다. 지금은 "이미 쓰고 있는 컷 N/M장 등록" 이 찍힌다.
- 외부 API는 **실제 한도**를 확인한다. Serper 429(5건/초)는 내가 서치풀로 질의를
  8배 늘리면서 만든 것이다.

### 연애박사 원고(`079ae8dd`) — 이미지 6장 확정

키노라이츠 공식 스틸 3장(3000x2000) 포함, 6장 전부 다른 컷, 캡션은 전부 실제 사진
기준으로 다시 썼다. 원고 본문은 사용자 결정으로 그대로 둔다.

**남은 것**: 참고 자료에 인스타 링크가 "공식 홍보 게시물"로 남아 있다(실제로는
재가공 계정). 원고를 다시 쓰지 않는 한 그대로다.
