...
`docs/ai-handoff/SPRINT_2_DESIGN.md` 14절.

**다음 작업: Sprint 3 구현 시작.** 설계와 결정이 모두 끝났다 —
`docs/ai-handoff/SPRINT_3_DESIGN.md`(결정 항목 5건 전부 승인 완료)의 14절 순서대로 진행하면
된다. 첫 작업은 검수 규칙 4종 구현(`src/workflows/review/`)이다.
...
체크포인트에서 사람이 원본 사이트를 직접 열어보는 것으로 당분간 대체. 승인 버튼(확인/수정/폐기)은
아직 의학 주제 전용이고, 비의학 원고 공통 승인 게이트는 Sprint 3에서 붙인다.

## Sprint 3: 검수 게이트 + 이미지 (설계 완료, 구현 대기)

설계와 결정이 전부 끝났다: `docs/ai-handoff/SPRINT_3_DESIGN.md`. 승인된 결정 5건:

1. 검수 결과는 **차단이 아닌 참고** — 오탐 데이터가 0인 상태에서 차단하면 멀쩡한 원고가 사라진다
2. 검수 이력은 새 테이블 대신 `article_jobs.metadata.reviewChecks`에 저장 (migration 수동
   적용 부담을 지금 질 이유가 없다. 나중에 커지면 테이블로 옮긴다)
3. 의학 전용 버튼(confirm/edit/discard)을 **모든 원고 공통 승인 버튼으로 승격** — confirm이
   이제 `job.status`를 `approved`로 전이시킨다(지금은 status를 안 건드림). ⚠️ 이미 confirm이
   눌린 job("아기 셔더링어택")은 `review`로 남아 있어 다시 눌러야 approved가 된다
4. 이미지는 **ChatGPT 수동 핸드오프** — Codex CLI는 코딩 에이전트라 이미지 생성이 불가하다.
   Claude가 브리프(장면 + 생성 프롬프트 + alt_text + 금지 항목)를 만들어 Telegram으로 보내고,
   사용자가 ChatGPT에서 확보한 뒤 업로드 직전 직접 검수한다
5. 이미지 **서치는 공식 보도자료·라이선스 명시 스톡으로만 한정**, 생성이 기본 — 웹 검색으로
   찾은 이미지는 로드맵 §6-1이 "가장 큰 실질 위험"으로 지목한 타인 저작물이고, 침해 여부는
   사람이 눈으로 봐도 판별되지 않는다(워터마크 없는 침해물이 대부분)

**구현 순서**(설계문 14절): ① 검수 규칙 4종(`src/workflows/review/`) → ② 결과를 알림에 표시
→ ③ `review:` 콜백을 공통 승인 흐름으로 승격 → ④ 실측 1건 검증 → ⑤ 이미지 브리프 생성 →
⑥ `images` 기록(+ `images.id`가 identity인지 첫 insert로 검증).

가장 까다로운 부분은 ①의 팩트 검사다: 근거의 "2026. 9. 2."와 본문의 "9월 2일"을 같은 값으로
인식하도록 정규화해야 하고, 그래도 못 맞추는 표기가 남으므로 "틀렸다"가 아니라 **"근거에서
확인되지 않음"**으로 표기해 사람이 최종 판단하게 한다.
