...
**발행 흐름 확정(2026-09-16 사용자 결정) + 임시저장 자동화 켬**:
1. **지금**: 텔레그램 최종 승인 → 원고·이미지 준비 → **Blogspot 초안 자동 저장**
   (`BLOGGER_ENABLED=true` + `BLOGGER_PUBLISH_AS_DRAFT=true`, 로컬 `.env`와 GitHub vars 모두 켬).
2. 사람이 Blogger UI에서 퍼머링크·검색 설명·웹 검색 이미지를 채우고 직접 공개.
3. **나중**: `BLOGGER_PUBLISH_AS_DRAFT=false`로 내리면 사람 영역을 건너뛰고 공개까지 자동
   (사람 영역은 발행 후 수정으로 채운다). ⚠️ 사용자가 "blogger_enabled를 켜면"이라고 불렀지만
   실제 스위치는 `BLOGGER_PUBLISH_AS_DRAFT`다 - `BLOGGER_ENABLED`는 1번을 위해 이미 켜져 있다.

⚠️ **3번으로 넘어가기 전에 반드시 고쳐야 할 것**: 지금은 채워지지 않은 웹 검색 마커가
