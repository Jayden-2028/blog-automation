...
**"시각적으로 일치"가 아니라 "정확히 일치" 탭을 쓴다**(2026-09-23 실측). 기본 화면은 비슷해
보이는 사진까지 담는다 - 암살자(들) 슬라이드(정우성·박정민 2인)에 **신유열 단독 증명사진**이
1위로 올라왔다. "얼굴 하나가 밝은 배경에" 정도만 맞아도 걸린다. 정확히 일치 탭이 없으면
포기한다. 시각적 유사로 내려가면 엉뚱한 사진을 "같은 사진"이라고 내주게 된다.

**대부분의 뉴스카드 슬라이드는 정확히 일치가 없다.** 구글이 그렇게 답한다 - "일치검색 결과를
찾을 수 없음. 이미지가 고유하거나 아직 널리 공유되지 않았다는 의미일 수 있습니다". 당연하다:
그 슬라이드는 **게시자가 만든 합성물**이다(인물 사진 둘 + 텍스트 띠 + 로고). 그 합성물은 그
게시물에만 있다. 그래서 2-b는 **원본 보도사진을 그대로 올린 게시물에서만** 걸리고, 나머지는
2-a로 내려간다. 이것은 버그가 아니라 이 방법의 한계다.

**지각 해시(dHash)로 슬라이드를 직접 대조하는 것은 안 된다.** 슬라이드는 1:1 뉴스카드(+텍스트
띠)이고 기사 og:image는 1.9:1 크롭이라, dHash가 가로세로를 뭉개는 특성상 같은 사건의 사진도
거리 25~41로 벌어진다. 해시는 **정확히 일치로 이미 좁혀진 페이지 안에서 어느 이미지인지 고를
때만** 쓴다(렌즈 카드 썸네일 기준, 상한 12).
