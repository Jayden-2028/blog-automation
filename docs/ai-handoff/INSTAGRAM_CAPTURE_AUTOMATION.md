...
**렌즈는 받을 수 있는 이미지 주소를 주지 않는다**(2026-09-23 실측). 결과 카드의 썸네일은
205px짜리 `data:` URI이고 `<a>` 바깥에 있다. 그래서 렌즈에서는 **출처 페이지만** 얻고,
실제 이미지는 그 페이지의 `og:image`에서 받는다 - 기사 og:image는 원본 해상도이고 링크
미리보기용이라 구조가 안정적이다(캡션을 og:description에서 뽑기로 한 것과 같은 이유).

og:image 값은 HTML 속성이라 엔티티를 푼다. X(트위터)가 `?format=webp&amp;name=large`로
주는데 그대로 받으면 404다.
