...
export function buildBriefPrompt(input: BuildKeywordBriefInput): string {
  ...
  return [
    ...
    "- Q1은 대개 '이게 뭔가/누군가'(정의)다. **다만 이미 답이 나와 있으면 그 칸을 쓰지 마라**",
    "  (2026-09-22 실측: 키워드가 '낼 모레 50인데 너무 예뻐서 20대로 오해받는 여배우'인데 Q1을",
    "  '이 여배우가 누구인가'로 잡았다 - 제목 한 줄이면 끝나는 것을 질문 한 칸으로 썼다).",
    "  이름이 널리 알려진 인물이거나 제목·헤드라인에 답이 있으면, 그 칸은 독자가 진짜 궁금한 것에 쓴다.",
    "- 마지막 질문은 항상 '그래서 나는 뭘 할 수 있나'(행동)다 - policy/event/product는 신청·예매·관람·구매,",
    "  drama는 어디서 보나, celebrity/incident는 앞으로 뭘 지켜볼지.",
    "- **질문끼리 겹치면 안 된다.** 같은 사실을 각도만 바꿔 두 번 묻지 마라(실측: '어떤 계기로 화제가",
    "  퍼졌나'와 '실물이 어땠길래 이런 반응이 나왔나'는 사진 한 장을 두 번 묻는 것이다). 겹친다 싶으면",
    "  하나로 합치고 남는 칸에 다른 질문을 넣는다.",
    "- **'왜 화제인가'는 출연진 조합이 아니라 독자가 실제로 반응한 지점을 묻는 것이다.** 제목에 이름 둘이",
    "  같이 나온다고 '케미가 화제'로 단정하지 마라 - 사람들이 어떤 장면·감정에 반응했는지를 묻는다.",
    "- 기사에 답이 있을 것 같은 질문이 아니라 **독자가 궁금한 질문**을 쓴다. 답이 없을 수도 있다 -",
    "  그건 리서치가 확인할 일이지 네가 걸러낼 일이 아니다.",
    "",
    "## 제목만 보고 단정하지 마라 (2026-09-22 실측 사고)",
    "너에게는 **제목만** 주어진다. 본문이 없다. 한국어 헤드라인은 작품명을 줄여 쓰고 인물명을 바로 붙여서,",
    "제목만으로는 주어를 가릴 수 없는 경우가 많다.",
    "",
    "```",
    "제목: \"'로또 1등' 서현우, 제대로 사고쳤다\"",
    "잘못 읽음: 서현우가 로또 1등에 당첨됐다",
    "실제:      '로또 1등'은 드라마 제목 약칭이고, 당첨자는 이준혁이다",
    "```",
    "",
    "이 오해가 그대로 질문이 되면, 리서치가 사실을 바로잡아도 **틀린 각도는 원고에 남는다**",
    "(실측: 브리프만 헷갈린 것이 '많은 사람이 헷갈리는 지점'이라는 소제목과 FAQ 한 칸이 됐다).",
    "확실하지 않으면 **단정하는 대신 그것 자체를 질문으로 만들어라** - \"당첨된 인물은 누구인가\"처럼.",
    "",
    "## 출력 (이 형식만, 다른 말 없이)",
