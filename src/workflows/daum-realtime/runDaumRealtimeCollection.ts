// 다음 실시간 트렌드 수집 -> trend_candidates 저장까지의 배선(collection) 워크플로우.
...
import { DAUM_REALTIME_SOURCE, mapDaumRealtimeItemsToInserts } from "./mapDaumRealtimeCandidates.js";
import type { FetchDaumRealtimeResult } from "../../services/search/providers/daumRealtime/DaumRealtimeProvider.js";

export type RunDaumRealtimeCollectionOptions = {
  /** 생략하면 TREND_SOURCE_CONFIGS.daum_realtime.enabled. false면 즉시 skipped. */
  enabled?: boolean;
  ...
  fetchTrends?: () => Promise<FetchDaumRealtimeResult>;
};

export type RunDaumRealtimeCollectionResult = {
  ...
  /** 중복/너무 짧은 키워드로 버려진 수. */
  droppedCount: number;
  /** 제외 카테고리(육아 등)·정치 키워드로 걸러져 저장되지 않은 수. */
...
export async function runDaumRealtimeCollection(
  options: RunDaumRealtimeCollectionOptions = {}
): Promise<RunDaumRealtimeCollectionResult> {
  const enabled = options.enabled ?? TREND_SOURCE_CONFIGS.daum_realtime.enabled;
  ...
  try {
    const fetchTrends = options.fetchTrends ?? (() => fetchDaumRealtime());
    const fetched = await fetchTrends();

    // 매핑은 순수 함수이므로 DB 접근 전에 끝낸다 - 매핑에서 실패하면 원격에 아무것도 쓰지 않는다.
    const { rows: mapped, droppedCount } = mapDaumRealtimeItemsToInserts(fetched.items, {
    ...
    const expiredCount = options.skipExpire
      ? 0
      : await TrendCandidateRepository.expireOldCandidates({ source: DAUM_REALTIME_SOURCE });
    ...
    console.error("⚠️ [runDaumRealtimeCollection] 수집 실패 -", message);
    return {
      status: "failed",
      fetchedCount: 0,
      upsertedCount: 0,
