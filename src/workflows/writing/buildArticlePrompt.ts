...
// INCIDENT(사건·사고): 개인 톤을 절대 쓰면 안 되는 유일한 카테고리다.
//
// 왜 별도인가(2026-09-04): 사망·범죄·재난 기사에 PERSONAL_STYLE_RULES가 적용되면 "저도 궁금해서
// 찾아봤어요", "~더라고요", 끝에 개인적 소감 문단 같은 지시가 그대로 들어간다. 유족이 있는 사건에
// 그런 톤으로 쓰는 것은 그 자체로 사고다. EDITORIAL도 "명확한 입장·제언"과 "자신감 있고 단정적인
// 어조"를 요구해서 수사 중인 사건에는 맞지 않는다(단정이 곧 명예훼손 위험이다).
//
// COMMON_RULES가 이미 추측성 표현 금지와 명예훼손 경고를 담고 있지만, 그것은 "지어내지 마라"이고
// 여기는 "어떤 태도로 쓰느냐"다. 둘 다 필요하다.
const INCIDENT_STYLE_RULES = [
  "1인칭을 쓰지 않는다. 개인 경험담, \"저도 궁금해서\", 개인적 소감 문단을 넣지 않는다 -" +
    " 피해자와 유족이 있는 사안이다.",
  "모든 문장을 \"~습니다/합니다\"체로 통일한다. \"~해요\", \"~네요\", \"~더라고요\" 같은 가벼운" +
    " 구어체 종결을 쓰지 않는다.",
  "수사·재판이 진행 중인 사안은 확정된 사실과 아직 확정되지 않은 것을 문장 단위로 구분해 쓴다." +
    " 혐의를 기정사실로 단정하지 않는다(무죄추정).",
  "피해자·유족의 신원을 추정할 수 있는 세부(실명, 직장, 거주지, 학교 등)는 자료조사 파일에 있더라도" +
    " 본문에 옮기지 않는다.",
  "사건의 자극적 세부(사망 경위 묘사, 현장 정황 묘사)를 재현하지 않는다. 무엇이 일어났는지" +
    " 사실 관계만 전달한다.",
  "클릭을 노린 감정 자극형 제목·소제목을 쓰지 않는다. 사건을 소재로 흥미를 유발하지 않는다.",
  "본문 끝에 개인적 소감 대신, 확인된 사실의 요약과 후속 절차(수사·재판 일정 등 자료에 있는 경우)만" +
    " 담담하게 정리한다.",
] as const;
 ...
 *
 * incident(2026-09-04 추가)는 PERSONAL로 폴백되면 안 되는 유일한 category다 - 위 주석 참고.
 */
export function pickStyleRules(category: string | null): readonly string[] {
  if (category === "incident") return INCIDENT_STYLE_RULES;
