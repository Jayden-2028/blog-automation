...
// articles 행으로 만든다. 본문의 `## ` 소제목과 `[IMAGE: 설명]` 마커는 그대로 보존한다
// (다운스트림 변환기가 `##`를 쓰고, 이미지 마커는 사용자가 직접 채운다).
...
export type ParsedDraftFile = {
  ...
  /** frontmatter/해시태그 줄/HTML 주석을 걷어낸 순수 본문(마크다운, `##`·`[IMAGE:]` 유지). */
  body: string;
...
export function parseDraftFile(text: string): ParsedDraftFile {
  ...
  const { stripped: noHashtags, hashtags } = extractHashtags(noComments);
