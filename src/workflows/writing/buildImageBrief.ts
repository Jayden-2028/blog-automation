...
function buildPrompt(input: BuildImageBriefInput): string {
  ...
  return [
    ...
    "- PROMPT(영어): 이미지 생성 모델에 그대로 붙여넣을 프롬프트 한 줄. 실사(photorealistic) 사진" +
      " 스타일로 요청한다(예: photorealistic, DSLR photo, natural lighting, shallow depth of" +
      " field) - 일러스트·플랫 디자인·만화체로 만들지 않는다",
    "- 사람이 등장해야 자연스러운 장면이면 얼굴이 뚜렷하게 보이지 않는 구도(뒷모습, 손만 보이는" +
      " 클로즈업, 얼굴이 흐릿하게 나오는 원거리 등)로 지정한다 - 실사 스타일에서 특정 인물처럼" +
      " 보이는 얼굴이 생성되는 위험을 줄인다",
    "- 브랜드·제품이 주제여도 로고나 글자가 보이는 실제 포장·간판을 요청하지 않는다. \"플레인/무지" +
      " 포장(plain unbranded packaging)\", \"로고 없음(no visible logos or text)\"을 프롬프트에" +
      " 명시한다 - 실사 스타일일수록 실제 브랜드 자산처럼 보일 위험이 커진다",
    "- ALT_TEXT(한국어): 접근성 및 SEO용 대체 텍스트 한 줄",
    "- PROHIBITED(한국어): 이 이미지에 들어가면 안 되는 것을 쉼표로 나열한다. 실존 인물, 특정" +
      " 브랜드 로고·문구, 저작권이 있는 캐릭터는 항상 포함한다",
