...
export function buildNaverVariantPrompt(input: GenerateNaverVariantInput): string {
  ...
  return [
    ...
    "",
    "## 절대 규칙 - 사실 보존",
    "수치·날짜·금액·고유명사·인용·제도명은 원본 그대로 쓴다. 새 사실을 추가하지 않는다(웹 검색 없음).",
    "",
    "## 절대 규칙 - '참고 자료' 블록은 통째로 뺀다 (2026-09-22 사용자 결정)",
    "`**참고 자료**` 소제목과 그 아래 바깥 링크 목록을 **전부 삭제**한다. 네이버는 외부로 나가는",
    "링크를 본문 끝에 몰아 두면 이탈로 이어져, 블로그와 달리 여기서는 두지 않기로 했다.",
    "`**요약**` 블록은 그대로 둔다 - 독자에게 실용적이다.",
    "본문 안에서 출처를 문장으로 밝히는 것(\"부산일보 인터뷰에 따르면\")은 그대로 유지한다.",
    "빠지는 것은 **링크 목록뿐**이고, 사실의 출처를 감추라는 뜻이 아니다.",
    "",
    "## 무엇을 바꾸는가",
