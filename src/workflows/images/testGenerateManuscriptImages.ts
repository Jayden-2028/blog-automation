// generateManuscriptImages 테스트. 이미지 API(OpenAI/Gemini)와 Supabase Storage를 전부 주입해
// 짝짓기·A/B·실패 격리·상한만 검증한다. 외부 호출·유료 호출 없음.
...
const okGenerate = async (_input: { prompt: string }, provider?: string) => ({
  ...
  mimeType: "image/png",
  provider: (provider ?? "openai") as "openai" | "gemini",
});
...
async function main(): Promise<void> {
  ...
  const partial = await generateManuscriptImages(input, {
    ...
    generate: async (i) => {
      n += 1;
      return n === 1 ? { ok: false as const, error: "rate limit" } : okGenerate(i);
    },
    upload: okUpload,
  });
