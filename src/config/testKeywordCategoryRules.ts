...
const CASES: readonly (readonly [string, string, string])[] = [
  ...
  // --- community(2026-08-29 추가) ---
  // 규칙 순서가 지켜지는지가 핵심이다: community는 맨 마지막이므로, 앞 네 규칙이 잡는 키워드는
  // 커뮤니티 어휘가 섞여 있어도 절대 community가 되면 안 된다. "출처가 아니라 내용으로 판정한다".
  ["연예인 갑질 논란 누리꾼 갑론을박", "일상·생각", "entertainment"],  // "갑질/누리꾼"보다 "논란"? -> entertainment 규칙 우선
  ["드라마 결말 두고 시청자 갑론을박", "일상·생각", "ott"],             // "드라마"가 먼저 잡는다
  ["아기 훈육 방식 두고 맘카페 설전", "일상·생각", "parenting"],        // "아기"가 먼저 잡는다
  ["지원금 지급 기준 공분", "일상·생각", "living"],                     // "지원금"이 먼저 잡는다
  // 앞 네 규칙이 전부 미스한, 순수 인터넷 화제/논쟁만 community가 된다.
  ["요즘 유행어 뜻 정리", "일상·생각", "community"],
  ["출근길 빌런 목격담 화제", "일상·생각", "community"],
  ["국민청원 20만 돌파", "일상·생각", "community"],
  ["MZ 밈 챌린지 확산", "일상·생각", "community"],
] as const;
