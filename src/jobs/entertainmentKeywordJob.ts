...
// job이 09:00에 이미 Creator Advisor 크롤링 + 구글 트렌드 조회를 끝내고 trend_candidates에 오늘자
// 후보를 채워 두므로, 이 job은 **크롤링을 다시 하지 않는다**(trendCollectOptions/googleTrendsOptions
// 를 enabled:false로 꺼서 소스 재수집을 건너뜀) - Creator Advisor 브라우저 자동화를 하루 두 번
// 돌리는 건 로그인 세션·rate limit 부담만 키운다. 대신 collectionSources에는 두 소스 이름을 그대로
// 남겨 buildDailyQueryPool()이 오늘자 trend_candidates를 읽게 하고, includeCategories로 연예·OTT만
// 추려 별도의 Top N + Telegram 알림을 만든다. 그래서 이 job은 **반드시 socialIssueKeywordJob 이후에
// 실행돼야 한다**(launchd 09:00 / 09:10 순서로 보장).
